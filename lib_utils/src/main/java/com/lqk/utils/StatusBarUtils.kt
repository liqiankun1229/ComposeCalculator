...
object StatusBarUtils {
    ...
    fun setColorForSwipeBack(activity: Activity, @ColorInt color: Int,
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
            ...
            if (rootView != null && rootView is CoordinatorLayout) {
                val coordinatorLayout: CoordinatorLayout = rootView
                if (Build.VERSION.SDK_INT < Build.VERSION_CODES.LOLLIPOP) {
                    coordinatorLayout.fitsSystemWindows = false
                    contentView.setBackgroundColor(calculateStatusColor(color, statusBarAlpha))
                    val isNeedRequestLayout = contentView.getPaddingTop() < statusBarHeight
     ...
     */
    @JvmOverloads
    fun setTranslucent(activity: Activity, @androidx.annotation.IntRange(from = 0, to = 255) statusBarAlpha: Int = DEFAULT_STATUS_BAR_ALPHA) {
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.KITKAT) {
            return
        }
