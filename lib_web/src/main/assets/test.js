...
!function () {
    ...
    window.coreJSBridge = {
        ...
        isLQK: function () {
            /* 在 集成 lqkBridgeSDK 的APP 中 -> ua 中加入关键字 */
            return true;
        },
        check: function () {
            /* 校验信息 */
            ...
            if (typeof callback === "object") {
                if (typeof callback.onSucceed === "function" && typeof callback.onFailed === "function") {
                    ...
                    if (this.isLQK()) {
                        ...
                        /* 构造传递对象 */
                        lqkParams[keyParams.funcName] = funName;
                        lqkParams[keyParams.params] = params;
                        lqkParams[keyParams.callbackName] = key;
                        ...
                        if (this.isAndroid) {
                            ...
                            window.lqkCore.onNative(JSON.stringify(lqkParams));
                        } else if (this.isIOS()) {
                            /* iOS 环境 */
                            window.webkit.messageHandlers.core.postMessage(JSON.stringify(lqkParams));
                        } else {
                            /* 不是app 中,只打印入参 log */
                            console.log(JSON.stringify(lqkParams));
                        }
                    } else {
                        isDebug && console.log("----------不是 LQK 环境----------");
                        return 0
                    }
                } else {
