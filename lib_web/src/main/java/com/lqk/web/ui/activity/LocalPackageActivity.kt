...
import com.lqk.web.conf.Config.BASE_URL
...
import com.lqk.web.ui.widget.web.OfficeActivity
...
import com.lqk.web.weex.WeexActivity
...
class LocalPackageActivity : BaseVBActivity<ActivityLocalPackageBinding>() {
    ...
    companion object {
        ...
        const val x5_URL = "http://119.96.3.31/soft.imtt.qq.com/browser/tes/feedback.html"
    }
    ...
    override fun initView() {
        ...
        if (!TbsFileInterfaceImpl.isEngineLoaded()) {
            TbsFileInterfaceImpl.initEngineAsync(this@LocalPackageActivity, object :ITbsReaderCallback{
                override fun onCallBackAction(p0: Int?, p1: Any?, p2: Any?) {
                    Log.d(TAG, "onCallBackAction: 插件 p0:$p0 p1:$p1 p2:$p2")
                }
            })
        }
    }
                    ...
                    0 -> {
                        if (searchPackage(versionInfo)) {
                            ...
                            if (!PackageUtils.checkPackageVersion(versionInfo)) {
                                when(versionInfo.category){
                                    "web"-> openWebLocal(versionInfo)
                                    "weex" -> openWeex(versionInfo)
                                    else ->{
                                        Toast.makeText(
                                            this@LocalPackageActivity,
                                            "其他容器暂不支持",
...
//                                                )
                                                when(versionInfo.category){
                                                    "web"-> openWebOnline(versionInfo)
                                                    "weex" -> openWeexOnline(versionInfo)
                                                    else ->{
                                                        Toast.makeText(
                                                            this@LocalPackageActivity,
                                                            "其他容器暂不支持",
...
//                WebActivity.startLocal(this, "file:$htmlPath")
                openWebLocal(versionInfo = it)
            }
        }
        
        // Web
        viewBinding.btnOpenUrl.setOnClickListener {
            var url = viewBinding.etUrl.text.toString()
        ...
        //
        viewBinding.btnOpenWeexUrl.setOnClickListener {
            var url = viewBinding.etWeexUrl.text.toString()
            if (!(url.startsWith("http") || url.startsWith("file"))) {
                url = "$BASE_URL$url"
            }
            WeexActivity.start(this, url)
        }
        ...
        viewBinding.btnOffice.setOnClickListener {
            OfficeActivity.start(this@LocalPackageActivity, "/sdcard/test.docx")
        }
        viewBinding.btnOfficeDocx.setOnClickListener {
            OfficeActivity.start(this@LocalPackageActivity, "/sdcard/test.docx")
        }
        viewBinding.btnOfficeXlsx.setOnClickListener {
            OfficeActivity.start(this@LocalPackageActivity, "/sdcard/test.xlsx")
        }
        viewBinding.btnOfficePdf.setOnClickListener {
            OfficeActivity.start(this@LocalPackageActivity, "/sdcard/test.pdf")
        }
        ...
        viewBinding.btnWeex.setOnClickListener {
...
//                "$BASE_URL:5000/zip/info",
    ...
    private fun openWebLocal(versionInfo: VersionInfo) {
        val path = cachePackage()
        val htmlPath = "$path/${versionInfo.appId}/${versionInfo.home}"
        WebActivity.startLocal(this, "file:$htmlPath")
    }
    
    private fun openWebOnline(versionInfo: VersionInfo) {
        WebActivity.startLocal(this, versionInfo.onlineUrl)
    }
    
    private fun openWeex(versionInfo: VersionInfo){
        val path = cachePackage()
        val htmlPath = "$path/${versionInfo.appId}/${versionInfo.home}"
        WeexActivity.start(this, "file:$htmlPath")
    }
    
    private fun openWeexOnline(versionInfo: VersionInfo){
        WeexActivity.start(this, versionInfo.onlineUrl)
    }
    ...
    private fun doUnZipFile(versionInfo: VersionInfo) {
        thread {
            ...
            if (file.exists()) {
                ...
                zipFile.close()
                runOnUiThread {
                    Toast.makeText(this, "${versionInfo.appId} 离线包下载完毕", Toast.LENGTH_SHORT)
                        .show()
                    checkVersion()
                }
            } else {
...
//        Log.d(TAG, "downVersion: $filePath")
    ...
    private fun showPackages(list: MutableList<VersionInfo>) {
        ...
        PackageUtils.useNetworkPackageInfo(list)
    }
