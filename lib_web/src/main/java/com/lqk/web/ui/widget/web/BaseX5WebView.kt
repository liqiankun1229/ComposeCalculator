...
//        this.loadNativeJavaScriptInterface<NativeJavaScriptInterface>(mountWebString(), native)
    ...
    private var strMountObj = "lqkCore"
...
//                    .show()
            ...
            override fun onShowFileChooser(
                ...
                fileChooserParams?.let {
                    ...
                    when (acceptType[0]) {
                        "image/*" -> {
                            VideoOrImageUtil.openImage(this@BaseX5WebView.context, object :
                        ...
                        "video/*" -> {
                            VideoOrImageUtil.openVideo(this@BaseX5WebView.context, object :
                                OnResultCallbackListener<LocalMedia> {
                                override fun onResult(result: MutableList<LocalMedia>?) {
                                    result?.let { m ->
                                        val uriList: MutableList<Uri> = mutableListOf()
                        ...
                        "audio/*" -> {
                            VideoOrImageUtil.openImageOrVideo(this@BaseX5WebView.context, object :
                                OnResultCallbackListener<LocalMedia> {
                                override fun onResult(result: MutableList<LocalMedia>?) {
                                    result?.let { m ->
                                        val uriList: MutableList<Uri> = mutableListOf()
...
//                loadJsFromLocal("jssdk-web-1.0.0.js")
                ...
                this@BaseX5WebView.webCallback?.onPageFinish()
            }
