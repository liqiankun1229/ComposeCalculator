            ...
            "scanCode" -> {
                ...
                AlertDialog.Builder(this)
                    ...
                    .setNegativeButton("确定") { _, _ ->
                        run {
                            callback?.onSucceed(
                                linkedMapOf("url" to "https://www.baidu.com/"),
                                callbackName
                            )
                        }
                    }
...
//                    callbackName
//                )
                startActivity(Intent(this, com.lqk.web.ui.activity.UPPayWebActivity::class.java))
            }
            else -> {
                // 未知字段 用户处理 不处理则不做操作
...
//            var s =  getNetWorkInfo()
    ...
    open fun initWebListener() {
        // 修改关键字
        baseWebView?.resetMountStr("lqkCore")
    ...
    open fun bindMenu() {
        ...
        // jsbridge
        baseWebView?.loadJs("coreJSBridge.onWeb", Gson().toJson(param)) {
...
//        Log.d(TAG, "getIMEI: ${PhoneUtils.getIMEI(this)}")
        ...
        Log.d(TAG, "getDeviceFingerprint: ${PhoneUtils.getDeviceFingerprint()}")
...
//        val url = intent.getStringExtra(KEY_URL) ?: "https://app.xjbsefjz.fun:11180/sdk-demo/#/"
        val url = intent.getStringExtra(KEY_URL) ?: "http://10.100.144.175:8081/"
        if (url.isEmpty()) {
            // 没有 url 地址, 页面没必要启动
            finish()
        }
