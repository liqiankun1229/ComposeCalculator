    ...
    private fun initWebViewClient(): WebViewClient {
        return object : WebViewClient() {
            ...
            override fun shouldOverrideUrlLoading(p0: WebView?, p1: String?): Boolean {
                Log.d(TAG, "shouldOverrideUrlLoading: Str $p1")
                return super.shouldOverrideUrlLoading(p0, p1)
            }

            override fun shouldOverrideUrlLoading(p0: WebView?, p1: WebResourceRequest?): Boolean {
                Log.d(TAG, "shouldOverrideUrlLoading: webResource $p1")
...
//                if (webResourceRequest?.url?.equals("${Config.BASE_URL}:5000") == true) {
                val uri = webResourceRequest?.url
                val strUrl = uri?.toString() ?: ""
//                if (strUrl.startsWith("${Config.BASE_URL}:8081")
                ...
                if (webResourceRequest?.method?.equals("POST") == true) {
                    ...
                    if (strUrl.startsWith("file://${"/zip"}")) {
                        // 更换链接
                        // file:///zip -> http://192.168.1.31:5000/zip

                        val data = OkHttpUtil.instance.doSyncPost(
//                        strUrl.replace("${Config.BASE_URL}:8081","${Config.BASE_URL}:5000"),
                            strUrl.replace(
                                "file:///zip",
                                "http://192.168.1.31:5000/zip"
                            ),
                            hashMapOf(),
                            String::class.java
                        )
                        ...
                        // 更换链接
                        // smk:///zip -> http://192.168.1.31:5000/zip
...
//                        strUrl.replace("${Config.BASE_URL}:8081","${Config.BASE_URL}:5000"),
            ...
            override fun onPageFinished(p0: WebView?, p1: String?) {
                ...
                if (p0?.progress != 100){
                    Log.d(TAG, "onPageFinished: 未加载完成 ${p0?.progress} : $p1")
                    return
                }
                Log.d(TAG, "onPageFinished: ${p0?.progress} : $p1")
                // native
//                loadJsFromLocal("jssdk-web-1.0.0.js")
    ...
    fun crosPost(webResourceRequest: WebResourceRequest): WebResourceResponse? {
        val uri = webResourceRequest?.url
        val strUrl = uri?.toString() ?: ""
        if (strUrl.startsWith("http://192.168.1.31:8081")
...
//                        header["Access-Control-Allow-Origin"] = "${Config.BASE_URL}:8081"
